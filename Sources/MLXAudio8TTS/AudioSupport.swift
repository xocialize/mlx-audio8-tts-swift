...
//
// Lifted verbatim from mlx-gepard-swift (same fleet pattern, same DSP need: decode an
// arbitrary reference clip to mono, resample it to the codec rate, encode the result as
// .wav). Only the temp-file prefix differs. Audio8's codec runs at 44.1 kHz.
...
enum AudioSupport {
    ...
    static func decodeToMono(_ audio: Audio) throws -> (samples: [Float], sampleRate: Int) {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("audio8-ref-\(UUID().uuidString).wav")
    ...
    /// Peak-normalized RMS in dBFS of mono samples — the app harness's non-silence probe
    /// (a silent stem reads −∞; don't trust ears). Returns -.infinity for empty/silent input.
    static func dBFS(_ samples: [Float]) -> Double {
